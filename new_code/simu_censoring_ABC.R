...
simu <- function(n, shape, rate, lambda, seed, R, R_a, R_b){
  ...
  # Simulate bootstrap samples from null fitted model
  sim_data <- NULL #just to initialize
  for(i in 1:R){
    sim_data[[paste0("time_",i)]] <- rgamma(n_events, shape = par0[1], rate =  par0[2])
    sim_data[[paste0("event_",i)]] <- rep(1, n_events)
  }
...
# save results
save(res, file = "simu_result_cens_C_3.Rdata")
...
mean((results$LR_obs-results$hat_mean)/
       sqrt(results$hat_variance)<qnorm(0.05))
