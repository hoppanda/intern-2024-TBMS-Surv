...
##### PROCEDURE FOR FRC3 vs GAMMA, LOG-LOGIS

R <- 1000
fit0 <- flexsurvspline(formula=Surv(time,event)~1,data=data, k=3)
fit1 <- flexsurvreg(formula=Surv(time,event)~1,data=data,dist="gamma")
fit2 <- flexsurvreg(formula=Surv(time,event)~1,data=data,dist="llogis")
par0 <- fit0$coefficients
knots0 <- fit0$knots
par1 <- exp(fit1$coefficients)
par2 <- exp(fit2$coefficients)
...
for(i in 1:R) {
  ...
  boot_fit2 <- flexsurvreg(formula=Surv(time,event)~1,
  ...
  boot_par2[i,]<-exp(boot_fit2$coefficients)
}
...
for(i in 1:R){
  ...
  mu1[i]<-log_like_rcs(par0,knots0,usedat) - 
    log_like(dgamma,pgamma,star_par1,usedat)
  mu2[i]<-log_like_rcs(par0,knots0,usedat) - 
    log_like(dllogis,pllogis,star_par2,usedat)
}
