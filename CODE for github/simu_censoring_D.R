################### GAMMA VS WEIBULL WITH EXP CENSORING ####################
...
simu <- function(n, shape, rate, lambda, seed, R){
  ...
  # fit censoring model
  fit_cens <- flexsurvreg(formula=Surv(time,1-event)~1,data=data,dist="exp") 
  
  # Simulate bootstrap samples from null fitted model and censoring fitted model
  sim_data_surv <- simulate(fit0, nsim=R)
  sim_data_cens <- simulate(fit_cens, nsim=R)
  
  # Combine together the bootstrap data
  sim_data <- sim_data_cens #just to initialize
  for(i in 1:R){
    surv_time <- sim_data_surv[[paste0("time_",i)]]
    cens_time <- sim_data_cens[[paste0("time_",i)]]
...
# save results
save(res, file = "simu_result_cens_D_3.Rdata")
#load("simu_result_cens_1.Rdata")

results <- as.data.frame(res)

# type I error
mean(results$LR_p_value<0.05)
mean(results$Cox_p_value<0.05)

# type I error normal approximation
mean(results$Cox_obs<qnorm(0.05))
mean((as.numeric(results$LR_obs)-as.numeric(results$hat_mean))/
       sqrt(as.numeric(results$hat_variance))<qnorm(0.05))
